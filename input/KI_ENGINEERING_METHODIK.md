...
**Begleitdatei:** `CLAUDE.md` als kurze, automatisch geladene operative Projektverfassung
...
|---|---|---|
| `CLAUDE.md` | kurze operative Regeln, Projektprofil, Hard Stops, Modus und Befehle | in jeder Claude-Code-Sitzung |
| `KI_ENGINEERING_METHODIK.md` | Begründungen, Auswahlregeln, Profile, Checklisten und Vorlagen | nur aufgabenbezogen lesen |

Die Langmethodik wird in `CLAUDE.md` nur als Codepfad erwähnt. Ein `@KI_ENGINEERING_METHODIK.md`-Import wäre kontraproduktiv, weil die gesamte Langfassung bei jedem Start Kontext verbrauchen würde.
...
- **Prüfnachweis:** Protokollstand 2026-07-28 und Clientverhalten gegen die Primärquellen in Kapitel 30 verifiziert; siehe 23.3.
...
4. operative Projektverfassung `CLAUDE.md`, Projektstand und freigegebene Roadmap;
...
|---|---|---|
| `CLAUDE.md` | operative Regeln und Projektprofil | Projektstart |
...
| Runbook | Start, Betrieb, Backup, Restore, Rollback | Deployment/Betrieb |
...
| Pflicht | Quelle |
|---|---|
| `CLAUDE.md` mit Steckbrief, Hard Stops und Autoritätsstufen | 5, 26.1 |
...
| Sicherheitskern: Secrets, PII, Live-Daten, geschützte Branches | 6 (CLAUDE.md), 22 |
...
- **Verdiente Autonomie:** Autonomie wird je Aufgabentyp erweitert, nachdem wiederholte Läufe dieses Typs manuell geprüft und für gut befunden wurden; der Stand ist dokumentiert (z. B. im Projektstand), nicht gefühlt.
...
- **WIP-Limit:** Es sind höchstens so viele Schreibfronten offen, wie der Eigentümer/Reviewer innerhalb des vereinbarten Zeitfensters ehrlich reviewen kann. Neue Fronten starten erst, wenn Reviews abfließen.
- **Kleine Diffs sind Politik:** Viele kleine, unabhängig prüfbare Tranchen schlagen einen großen Wurf; als Richtwert bleibt ein Diff in einer konzentrierten Sitzung vollständig lesbar.
- **Verifier vor Reviewer:** Ein frischer Verifier-Subagent prüft jeden Kandidaten gegen eine feste Rubrik (Korrektheit vs. Spec, Contracts/Frozen Zone, Security/Secrets, Testwirksamkeit, UX-Zustände, Doku) und liefert belegte Findings. Das reduziert menschliche Last, ersetzt aber bei R2/R3 niemals den menschlichen Review (5.5).
...
- **Run-Manifest:** Jeder autonome/parallele Lauf hinterlässt eine kleine maschinenlesbare Datei (JSON/YAML) mit: Run-ID, Modus, Autoritätsstufe, Modell(en), Plan-/Spec-Referenz, Start-/End-SHA, berührten Pfaden/Contracts, Gate-Ergebnissen, Tool-/MCP-Aufrufen mit externer Wirkung, Tokens/Kosten, Parks und Ergebnisstatus. Das Manifest speist die Kennzahlen aus 8.5 und 21.4.
...
Die Venue folgt Datenklassifizierung, Credentials und Seiteneffekt, nicht Bequemlichkeit.
...
**Projekt-Zielrevision: `2026-07-28`.**

Diese Revision ist der größte Umbau seit dem Start des Protokolls und bringt unter anderem:
...
- Das Projekt pinnt genau **eine** Zielrevision; sie steht im Projektsteckbrief und in der Server-Registry.
...
> Dieser Abschnitt ist ausdrücklich **zeitabhängig**. Vor Scaffold, Upgrade und Release wird der Stand gegen die Primärquellen in Kapitel 30 erneut verifiziert.
...
- Unbeaufsichtigte Läufe (11.3) nutzen ausschließlich M0 und M1.
...
- M2-/M3-Fähigkeiten in unbeaufsichtigten Läufen nutzen;
...
|---|---|
| `CLAUDE.md` | immer relevante Projektfakten und Verhaltensregeln |
...
Eine Regel wird nicht dauerhaft, nur weil sie einmal hilfreich schien.
...
### 28.1 Projektprofil für `CLAUDE.md`
...
Die folgenden Quellen wurden für Version 4.0 geprüft. Zeitabhängige Details vor Einsatz erneut verifizieren.
...
- [METR: Impact of Early-2025 AI on Experienced Open-Source Developer Productivity](https://metr.org/blog/2025-07-10-early-2025-ai-experienced-os-dev-study/)
